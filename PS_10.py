...
class Hand(object):
    ...
    def update(self, word):
        ...
        """
        for item in word:
            if self.handDict.get(item, 0) > 1:
                self.handDict[item] -= 1
            else:
                self.handDict.pop(item)
        # TODO
    ...
    def __eq__(self, other):
        ...
        """
        check = False
        if len(self.handDict) == len(other.handDict):
...
class Player(object):
    ...
    def __cmp__(self, other):
        ...
        they're equal.
        """
        return (self.points > other.points) - (self.points < other.points)
        # TODO
    def __str__(self):
        """
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        if hand_str in wordlist_rearr:
            return wordlist_rearr[hand_str]
        ...
        while(stat_len > 1):                 # comparing constructed words with the word in sorted dict word_list
            ...
            for key in visited:
                if len(key) == stat_len:
                    if key in wordlist_rearr:
                        #print key
                        return wordlist_rearr[key]
            stat_len -= 1
